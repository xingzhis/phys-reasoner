...
- **Sbatch to submit:** `scripts/perlmutter/smoke_tir_het.sbatch` — 3 het-groups (trainer 1 node × 4 GPU + rollout 5 nodes × 4 GPU + xverify 1 GPU). SLURM header is the canonical Perlmutter config, resolved 2026-04-20 — `-A m2651`, `-C gpu&hbm80g`, `-q premium` for all three groups. Full rationale in `scripts/perlmutter/README.md` §2. Don't swap these values without asking the user.
