...
> - The Qwen3 switch has been merged to parent `main` + verl `physcode` and verified end-to-end locally on Qwen3-0.6B. The feature branch `switch/qwen3-thinking` is kept for forensics but the live branches are now `main` / `physcode`.
...
> - **The §0 and §2.1 branch-checkout steps below are stale** (they say `switch/qwen3-thinking`). A fresh Perlmutter session should just `git checkout main` (parent) and `git -C verl checkout physcode`.
